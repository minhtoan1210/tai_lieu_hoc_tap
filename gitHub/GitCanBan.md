...
```bash
git config --global user.name "Minh Toan"
git config --global user.email minhtoan@gmail.com
```
...
#React-Awesome minhtoan
Host github-minhtoan.com
HostName github.com
User git
IdentityFile ~/.ssh/id_rsa_minhtoan
```

Ở trên thì mình có dùng 2 tài khoản Github, cái đầu tiên dùng với private key là `id_rsa` với Host là `github.com`, cái thứ hai là `id_rsa_minhtoan` với Host là `github-minhtoan.com`.
...
```bash
git clone git@github-minhtoan.com:dreact04072022/git-can-ban.git
```

Tương tự với những câu lệnh như `git remote add origin git@github-minhtoan.com:dreact04072022/git-can-ban.git`
