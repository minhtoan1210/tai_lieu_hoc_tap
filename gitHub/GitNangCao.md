...
3. **git prompt**
   ...
   - Git prompt là một tính năng hoặc tiện ích tích hợp vào dòng lệnh (terminal), giúp hiển thị thông tin liên quan đến trạng thái Git của một repository ngay trong prompt (dòng nhắc lệnh). Thông tin này thường bao gồm:
     - Tên nhánh hiện tại (branch name).
     - Trạng thái của repository, chẳng hạn như:
     - Có các thay đổi chưa được commit.
     - Có các tệp chưa được thêm vào Git (untracked files).
     - Có các thay đổi đã được staged (chờ commit).
     - Có sự khác biệt giữa nhánh cục bộ (local branch) và nhánh từ xa (remote branch).
     - Trạng thái merge, rebase hoặc cherry-pick nếu có.

4. **git cherrypick**
   - git cherry-pick là một lệnh Git được sử dụng để áp dụng một hoặc nhiều commit từ nhánh này sang nhánh khác mà không cần hợp nhất toàn bộ nhánh. 
   - Lệnh này rất hữu ích khi bạn muốn chọn lọc các thay đổi cụ thể từ lịch sử commit mà không cần kéo theo toàn bộ các commit khác.
